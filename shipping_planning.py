...
def load_all_data():
    ...
    # Process sales orders - WORKING PATTERN FROM shipping_planning__3_.py
    if not sales_orders_df.empty and len(sales_orders_df.columns) >= 10:
        col_names = sales_orders_df.columns.tolist()
        rename_dict = {}
        
        # First pass: flexible matching by column name
        for col in col_names:
            col_lower = col.lower()
            if 'internal' in col_lower and 'id' in col_lower and 'Internal ID' not in rename_dict.values():
                rename_dict[col] = 'Internal ID'
            elif 'status' in col_lower and 'Status' not in rename_dict.values():
                rename_dict[col] = 'Status'
        ...
        # Second pass: map by position for date columns
        if len(col_names) > 3:
            rename_dict[col_names[3]] = 'Order Start Date'
        if len(col_names) > 23:
            rename_dict[col_names[23]] = 'Customer Promise Date'
        if len(col_names) > 24:
            rename_dict[col_names[24]] = 'Projected Date'
        if len(col_names) > 25:
            rename_dict[col_names[25]] = 'Pending Approval Date'
        ...
        # Filter to Q4 2025 based on Order Start Date
